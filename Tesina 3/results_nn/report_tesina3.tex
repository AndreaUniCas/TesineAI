...
\begin{enumerate}
    \item Lettura del file CSV tramite \texttt{pandas}.
    \item Scarto della prima colonna (identificativo del campione).
    \item Mappatura dell'etichetta di classe: \texttt{P} $\to$ 1 (paziente),
          \texttt{H} $\to$ 0 (sano).
    \item Imputazione dei valori mancanti con la \textbf{mediana} di ciascuna
          feature, calcolata sul dataset corrente.
    \item Conversione dei dati in array \texttt{numpy} di tipo \texttt{float}.
\end{enumerate}
Una volta terminate le operazioni, la funzione restituisce i seguenti valori:
\begin{itemize}[leftmargin=2cm]
    \item \texttt{X} (\texttt{numpy.ndarray}): matrice delle feature di forma $(n\_samples, n\_features)$.
    \item \texttt{y} (\texttt{numpy.ndarray}): vettore delle etichette binarie.
    \item \texttt{feature\_names} (\texttt{list}): nomi delle colonne delle feature.
\end{itemize}
...
suddivisione casuale train/test del dataset, raccogliendo un insieme completo
di metriche. 
Verranno usati i seguenti parametri:
\begin{itemize}[leftmargin=2cm]
    \item \texttt{X} (\texttt{numpy.ndarray}): matrice delle feature.
    \item \texttt{y} (\texttt{numpy.ndarray}): vettore delle etichette.
    \item \texttt{mlp\_params} (\texttt{dict}): iper-parametri dell'MLP.
    \item \texttt{run\_seed} (\texttt{int}): seme casuale per la riproducibilità
          dello split e dell'inizializzazione del modello.
\end{itemize}

La funzione esegue le seguenti operazioni:
\begin{enumerate}
    \item Suddivisione del dataset in training (80\%) e test (20\%)
          tramite \texttt{train\_test\_split}.
    \item Aggiunta del \texttt{random\_state} agli iper-parametri e costruzione
          della pipeline.
    \item Addestramento della pipeline sul training set, con misurazione del
          tempo di esecuzione.
    \item Predizione delle etichette e delle probabilità sul test set.
    \item Calcolo delle metriche di valutazione.
\end{enumerate}
...
\begin{itemize}[leftmargin=2cm]
    \item \texttt{X}, \texttt{y}: dati e etichette (come sopra).
    \item \texttt{mlp\_params} (\texttt{dict}): iper-parametri dell'MLP.
    \item \texttt{run\_seed} (\texttt{int}): seme casuale per la suddivisione
          dei fold e l'inizializzazione del modello.
\end{itemize}

\textbf{Dettagli implementativi}:
\begin{itemize}
    \item Usa \texttt{StratifiedKFold} per mantenere la proporzione delle classi
          in ciascun fold.
    \item Esegue la valutazione in parallelo (\texttt{n\_jobs=-1}) per sfruttare
          tutti i core disponibili.
    \item La metrica di scoring è l'accuratezza.
\end{itemize}
...
\end{lstlisting}
Funzione principale di orchestrazione: esegue \texttt{N\_RUNS}
...
I parametri usati sono:
\begin{itemize}[leftmargin=2cm]
    \item \texttt{X}, \texttt{y}: dati e etichette.
    \item \texttt{mlp\_params} (\texttt{dict}): iper-parametri dell'MLP.
    \item \texttt{label} (\texttt{str}): etichetta identificativa della
          configurazione (usata nei log e nei risultati).
    \item \texttt{n\_runs} (\texttt{int}): numero di ripetizioni, default 30.
\end{itemize}

Vengono eseguite le seguenti operazioni:
\begin{enumerate}
    \item Per ciascuna run $r \in \{0, \ldots, n\_runs - 1\}$:
    \begin{itemize}
        \item Chiama \texttt{single\_run} con seme \texttt{SEED + r}.
        \item Chiama \texttt{cv\_run} con seme \texttt{SEED + r}.
    \end{itemize}
    \item Aggrega i risultati calcolando media e deviazione standard di
          accuratezza, CV score, tempi di esecuzione.
    \item Calcola la \textbf{matrice di confusione media} sulle \texttt{n\_runs} run.
    \item Identifica la run con l'AUC migliore e ne conserva la curva ROC.
    \item Stampa a console un log ogni 10 run e un riepilogo finale.
\end{enumerate}
...
Viene effettuata un'analisi sistematica dell'impatto dell'\textbf{architettura}
della rete neurale e della \textbf{funzione di attivazione} sulle prestazioni
del classificatore.

\textbf{Variabili esplorate}:
\begin{itemize}
    \item \textbf{Architetture} (8 configurazioni): reti con uno, due o tre
          strati nascosti, con dimensioni crescenti:
          \texttt{(200,)}, \texttt{(400,)}, \texttt{(600,)},
          \texttt{(400,200)}, \texttt{(600,300)}, \texttt{(800,400)},
          \texttt{(400,200,100)}, \texttt{(600,300,150)}.
    \item \textbf{Funzioni di attivazione} (4): \texttt{identity},
          \texttt{logistic}, \texttt{tanh}, \texttt{relu}.
\end{itemize}

\textbf{Parametri fissi}: solver \texttt{adam}, $\alpha = 0.001$,
learning rate iniziale $= 0.001$, early stopping attivo con
\texttt{validation\_fraction=0.15} e \texttt{n\_iter\_no\_change=10}.

\textbf{Numero di configurazioni testate}: $8 \times 4 = 32$.

\textbf{Valore restituito}: dizionario con chiave \texttt{"\{arch\}\_\{activation\}"}
e valore il dizionario di risultati di \texttt{run\_config}.
...
Viene effettuata un'analisi dell'effetto dei parametri legati all'\textbf{ottimizzazione}
e al \textbf{tasso di apprendimento} sulle prestazioni del modello.

\textbf{Architettura base fissa}: \texttt{(400, 200)}, attivazione \texttt{tanh}.

\textbf{Variabili esplorate}:

\begin{enumerate}
    \item \textbf{Learning rate e policy} (con solver SGD):
    \begin{itemize}
        \item Valori di LR: \texttt{0.0001}, \texttt{0.001}, \texttt{0.01}, \texttt{0.1}.
        \item Policy: \texttt{constant}, \texttt{invscaling}, \texttt{adaptive}.
        \item Combinazioni totali: $4 \times 3 = 12$.
    \end{itemize}

    \item \textbf{Solver} (con LR adattiva):
    \begin{itemize}
        \item \texttt{adam} (LR adattiva), \texttt{sgd} (LR adattiva),
              \texttt{lbfgs} (senza LR, early stopping disabilitato).
        \item Combinazioni totali: 3.
    \end{itemize}

    \item \textbf{Batch size} (con solver Adam):
    \begin{itemize}
        \item Valori: \texttt{16}, \texttt{32}, \texttt{64}.
        \item Combinazioni totali: 3.
    \end{itemize}
\end{enumerate}
...
\begin{itemize}
    \item Per \texttt{lbfgs}, i parametri \texttt{learning\_rate} e
          \texttt{learning\_rate\_init} vengono rimossi (non applicabili)
          e l'early stopping viene disabilitato.
\end{itemize}

\textbf{Valore restituito}: dizionario con chiave descrittiva della
configurazione e valore il dizionario di risultati di \texttt{run\_config}.
...
Viene effettuata un'analisi dell'effetto delle tecniche di \textbf{regolarizzazione}
sulla capacità del modello di generalizzare, con particolare attenzione
all'overfitting.

\textbf{Architettura base fissa}: \texttt{(400, 200)}, attivazione \texttt{tanh},
solver \texttt{adam}, LR iniziale \texttt{0.001}.

\textbf{Variabili esplorate}:

\begin{enumerate}
    \item \textbf{Regolarizzazione L2} ($\alpha$):
    \begin{itemize}
        \item Valori: \texttt{0.0001}, \texttt{0.001}, \texttt{0.01},
              \texttt{0.1}, \texttt{0.5}.
        \item Controlla la penalizzazione dei pesi della rete.
    \end{itemize}

    \item \textbf{Early Stopping}:
    \begin{itemize}
        \item Confronto tra \texttt{early\_stopping=True} e
              \texttt{early\_stopping=False}.
    \end{itemize}

    \item \textbf{Validation Split} (con early stopping attivo):
    \begin{itemize}
        \item Valori di \texttt{validation\_fraction}: \texttt{0.10},
              \texttt{0.15}, \texttt{0.20}.
    \end{itemize}

    \item \textbf{Patience} (\texttt{n\_iter\_no\_change}):
    \begin{itemize}
        \item Valori: \texttt{5}, \texttt{10}, \texttt{20}.
        \item Controlla quante iterazioni consecutive senza miglioramento
              sono tollerate prima dell'arresto anticipato.
    \end{itemize}
\end{enumerate}

\textbf{Numero totale di configurazioni}: $5 + 2 + 3 + 3 = 13$.
