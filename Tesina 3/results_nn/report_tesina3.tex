...
\end{lstlisting}
Funzione principale di esecuzione delle configurazioni: esegue \texttt{N\_RUNS} (30 volte come valore di default)
...
descrittive.
Per ciascuna run $r \in \{0, \ldots, n\_runs - 1\}$: viene chiamato \texttt{single\_run} e \texttt{cv\_run}con seme \texttt{SEED}.
...
L'\textbf{overfitting index} è calcolato come
...
\begin{itemize}[leftmargin=*]
  ...
  \item La scelta dell'attivazione ha un effetto modesto sull'accuracy assoluta,
...
\begin{itemize}[leftmargin=*]
  ...
  \item In ambito clinico i falsi negativi (pazienti non riconosciuti come tali)
...
\begin{itemize}[leftmargin=*]
  ...
  \item L'\texttt{alpha} di L2 fissato a $0.001$ non è sufficiente per contenere
...
\begin{itemize}[leftmargin=*]
  ...
  \item Learning rate nell'intervallo $[0.01, 0.1]$ con qualsiasi policy è
...
\begin{itemize}[leftmargin=*]
  ...
  \item L'accuracy di questo scenario è la più alta e stabile dei tre. Il dataset
...
\begin{itemize}[leftmargin=*]
  ...
  \item Nello Scenario 3 il fattore che impatta maggiormente l'overfitting è la
